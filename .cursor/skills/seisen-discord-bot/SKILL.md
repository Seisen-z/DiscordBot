...
- **Dashboard guild 429s**: `/api/bot/dashboard-guilds` returns manageable guild metadata and bot guild IDs from one cached, single-flight Discord lookup. Config-route Discord `@me` validation is also single-flight per token. Dashboard components must use this route instead of independently calling `/api/guilds_proxy` or Discord `/users/@me/guilds`.
