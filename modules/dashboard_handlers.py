...
async def on_dashboard_trigger(action: str, guild: discord.Guild, payload: dict):
    ...
    try:
        if action == "restock":
            ...
            sent_msg = await channel.send(content=final_content, embeds=embeds, files=files, view=view)

            # Auto-Reactions for Announcement
            raw_reactions = data.get("auto_reactions") or []
            if isinstance(raw_reactions, str):
                reactions_list = [r.strip() for r in raw_reactions.split(",") if r.strip()]
            elif isinstance(raw_reactions, list):
                reactions_list = [str(r).strip() for r in raw_reactions if r]
            else:
                reactions_list = []

            for react in reactions_list[:10]:
                try:
                    await sent_msg.add_reaction(react)
